...
async function runPull(parsed) {
  ...
  out.info('Obteniendo archivos remotos ...');


  const res = await fetch(`${apiBase}/repositories/mine`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) fail(`No se pudo obtener el repositorio (HTTP ${res.status}).`);

  const { repositories = [] } = await res.json();
  const repo = repositories.find((r) => r.id === repositoryId);

  if (!repo) fail(`Repositorio ${repositoryId} no encontrado en tu cuenta.`);

  const remoteFiles = repo.files || [];
  if (!remoteFiles.length) {
    out.info('El repositorio no tiene archivos remotos.');
    return;
  }
  ...
  out.info(`Archivos remotos: ${remoteFiles.length}`);
  console.log('');

  for (const remoteFile of remoteFiles) {
    const folder   = !remoteFile.carpeta || remoteFile.carpeta === 'raiz' ? '' : remoteFile.carpeta;
    const relPath  = folder ? `${folder}/${remoteFile.nombre}` : remoteFile.nombre;
    const localPath = path.join(cwd, relPath);



if (fs.existsSync(localPath) && manifest[relPath]) {
  const localHash = hashFile(localPath);


  const tmpPath = path.join(os.tmpdir(), `syshub-pull-check-${Date.now()}`);
  try {
    const downloadUrl = `${origin}${remoteFile.downloadUrl}`;
    const fileRes = await fetch(downloadUrl, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!fileRes.ok) { skipped++; out.skip(`${relPath}`); continue; }
    ...
    // Hash diferente — reemplazar
    process.stdout.write(`  [>] ${relPath} (actualizado) ... `);
    fs.copyFileSync(tmpPath, localPath);
    fs.rmSync(tmpPath, { force: true });
    manifest[relPath] = remoteHash;
    process.stdout.write('ok\n');
    downloaded++;
    continue;
  } catch (e) {
    ...
    try {
      ...
      if (!fileRes.ok) {
        process.stdout.write(`error (HTTP ${fileRes.status})\n`);
        continue;
      }
      ...
      manifest[relPath] = hashFile(localPath);
  ...
  await apiPost(`${apiBase}/repositories/${repositoryId}/pull`, {}, token).catch(() => {});

  console.log('');
  out.ok(`Pull completado — ${downloaded} descargado(s), ${skipped} sin cambios`);
}
...
function printHelp() {
  ...
  Comandos:
    ...
    push           Sube archivos nuevos o modificados al servidor
                   ADVERTENCIA: sobrescribe los archivos remotos con tu version local
                   
    commit         Registrar snapshot sin subir archivos
    
    log            Ver historial de commits	
    
    pull           Descarga archivos nuevos o actualizados del servidor
                   ADVERTENCIA: sobrescribe tus archivos locales con la version remota

  Opciones:
    --api-base     URL base del API 
